...
//The class Point represents a two-dimensional point
class Point
{
public:
  ...
  // member the functions for setting x, y to xval, yval respectively  
  void Set_X(int xval);
  void Set_Y(int yval);
...
//This class constructor sets X, Y to zero when no values are specified:
Point::Point()
{
  X = 0;
  Y = 0;
...
// The Set_X sets the value of X coordinate to xval
void Point::Set_X(int xval)
{
  X = xval;
} 

//The Set_Y sets the value of Y coordinate to yval
...
//declarion of class rectangle
class rectangle
{
    private:
      //it contaons 4 points which further contains two coordinates each
        Point A;
        Point B;
        Point C;
        Point D;
        int side1()
        {           //we are using the Get function on class pointer to get the values of the cordinates
...
rectangle::rectangle(Point lower,Point upper)//getting the lower and upper point and seting it in our rectangles point
{
  A=lower;
  C=upper;//to get an integer by Get function and to set it to our points by Set function
...
// Test the classes Point and Rectangle
int main()
{
// Declare a point using default class constructor (x = y = 0):
  Point p1;
...
//Lab 7 Exercises 4.2, 4.3. Testing of the class Rectangle goes here:  
    ...
    Point a(x1, y1);Point b(x2, y2);//to declare two points for the rectangle
    rectangle r2(a,b);//rectangle taking two points as input
    r2.print();//to call function of rectangle
    cout<<"Area of the rectangle is "<<r2.area()<<endl;//to call area function from class rectangle
  return 0;
}
